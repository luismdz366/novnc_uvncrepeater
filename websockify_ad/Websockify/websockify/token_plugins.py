...
class AuthServer():
    ...
    # El proceso de validacion del token ocurre en esta funcion.
    # El token llega en la URL como parametro 'adp'
    # llega en la url que inicia el cliente desde la aplicacion.
    ...
    def closed(self, s_id: str, status: str | int):
        """Function to request the close of the session to the AD application"""
        try:
            requests.get(url, params={"id": str(
                0), "status": status}, timeout=5)
        except requests.RequestException:
            pass


if __name__ == "__main__":
    # check the Confiloader load config in development mode

    print("HEllo")
    config_loader = INIConfigLoader(ConfigEnv.DEV)
    print(config_loader.load_config())
    config_loader2 = INIConfigLoader(ConfigEnv.PROD)
    print(config_loader2.load_config())
# class BasePlugin():
