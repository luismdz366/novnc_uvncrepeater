...
class ConfigEnv(StrEnum):
    """Definition of execution profiles for the WebSocket proxy server."""
...
# Active execution profile, set once CLI args are parsed in websockify_init()
_current_env: ConfigEnv = ConfigEnv.PROD


def get_current_env() -> ConfigEnv:
    """Return the execution profile the server was started with."""
    return _current_env
...
"""
    ...
    def validate_connection(self):
        ...
        host, port, s_id = self.get_target(self.server.token_plugin)
        if host == 'unix_socket':
    ...
    def new_websocket_client(self):
        ...
        try:
            ...
            self.log_message("Failed to connect to %s:%s: %s",
                             self.server.target_host, self.server.target_port, self.server.target_id, e)
            if self.server.target_id:
                self.server.token_plugin.closed(self.server.target_id, 0)
    ...
    def get_target(self, target_plugin):
        ...
        if self.host_token:
            ...
            if 'adp' in args and len(args['adp']):
                token = args['adp'][0].rstrip('\n')
            else:
                token = None
        if token is None:
            raise self.server.EClose("Token not present")

        result = target_plugin.lookup(token)
...
def websockify_init():
    ...
    parser.add_option("--env", type="choice", default=ConfigEnv.PROD.value,
                      choices=[e.value for e in ConfigEnv],
                      help="Use this flag to select execution environment (%s), "
                      "defaults to '%s'" %
                      (", ".join(e.value for e in ConfigEnv), ConfigEnv.PROD.value))
    ...
    # optparse only validated the raw string; convert it to the enum member
    opts.env = ConfigEnv(opts.env)
    global _current_env
    _current_env = opts.env
    root.info("Starting websockify with execution profile: %s",
              str(opts.env.value).upper())
